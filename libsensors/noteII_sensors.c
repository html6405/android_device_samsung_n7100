...
struct sensor_t smdk4x12_sensors[] = {
	...
	{ "LSM330DLC Gyroscope Sensor", "STMicroelectronics", 1, SENSOR_TYPE_GYROSCOPE,
		SENSOR_TYPE_GYROSCOPE, (2000.0f*3.14f/180.0f), ((70.0f / 4000.0f) * (3.14f / 180.0f)), 6.1f, 1190, 0, 0, SENSOR_STRING_TYPE_GYROSCOPE, 0, 0,
		SENSOR_FLAG_ON_CHANGE_MODE, {}, },
	{ "BMP180 Pressure Sensor", "Bosch", 1, SENSOR_TYPE_PRESSURE,
		SENSOR_TYPE_PRESSURE, 1260.0f, 1.0f, 1.0f, 66700, 0, 0, SENSOR_STRING_TYPE_PRESSURE, 0, 20000,
		SENSOR_FLAG_CONTINUOUS_MODE, {}, },
};

int smdk4x12_sensors_count = sizeof(smdk4x12_sensors) / sizeof(struct sensor_t);

struct noteII_sensors_handlers *noteII_sensors_handlers[] = {
...
int noteII_sensors_handlers_count = sizeof(noteII_sensors_handlers) /
	sizeof(struct noteII_sensors_handlers *);
	...
	struct hw_device_t** device)
{
	...
	smdk4x12_sensors_device->handlers = noteII_sensors_handlers;
	smdk4x12_sensors_device->handlers_count = noteII_sensors_handlers_count;
	smdk4x12_sensors_device->poll_fds = (struct pollfd *)
		calloc(1, noteII_sensors_handlers_count * sizeof(struct pollfd));

	p = 0;
	for (i = 0; i < noteII_sensors_handlers_count; i++) {
		if (noteII_sensors_handlers[i] == NULL || noteII_sensors_handlers[i]->init == NULL)
			continue;

		noteII_sensors_handlers[i]->init(noteII_sensors_handlers[i], smdk4x12_sensors_device);
		if (noteII_sensors_handlers[i]->poll_fd >= 0) {
			smdk4x12_sensors_device->poll_fds[p].fd = noteII_sensors_handlers[i]->poll_fd;
